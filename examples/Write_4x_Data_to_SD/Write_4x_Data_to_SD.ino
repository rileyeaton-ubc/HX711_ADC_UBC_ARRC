...
const int HX711_sck_4 = 9; // mcu > HX711 no 5 sck pin
...
const float CALIBRATION_VALUE_3 = LoadCell_3.getCal_LoadCellC();
const float CALIBRATION_VALUE_4 = LoadCell_4.getCal_LoadCellD();
...
void setup() {
  ...
  while ((loadcell_1_rdy + loadcell_2_rdy + loadcell_3_rdy + loadcell_4_rdy) < 4) { // run startup, stabilization and tare, all simultaneously
  ...
  if (loadcell_fail) {
    ...
    LoadCell_3.setCalFactor(CALIBRATION_VALUE_3); 
    LoadCell_4.setCalFactor(CALIBRATION_VALUE_4);
    while (!LoadCell_1.update());
    while (!LoadCell_2.update());
    while (!LoadCell_3.update());
    while (!LoadCell_4.update());
...
void loop() {
  ...
  if (newDataReady) {
    ...
    if (millis() > t + csv_write_delay) {
      ...
      int loadcell_3_val = round(LoadCell_3.getData());
      int loadcell_4_val = round(LoadCell_4.getData());
      ...
      if (csvLogFile) {
        ...
        csvLogFile.print(",");
        csvLogFile.print(loadcell_2_val);
        csvLogFile.print(",");
        csvLogFile.print(loadcell_3_val);
        csvLogFile.print(",");
        csvLogFile.println(loadcell_4_val);
